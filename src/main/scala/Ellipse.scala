...
import scalafx.scene.control.{Button, CheckBox, ColorPicker, Label, TreeItem}
import scalafx.scene.layout.VBox
import scalafx.scene.paint.Color
import scalafx.Includes._
import Constants._

class Ellipse(var x: Double, var y: Double, var width: Double, var height: Double, var color: Color, var lineWidth: Double, var fill: Boolean, val drawing: Drawing) extends Drawable{
  ...
  def draw(graphics: GraphicsContext, forceSelect: Boolean) = {
    if(selected || (this.isSelected && Main.mode != "Select") || forceSelect) {
      ...
      graphics.fillOval(x-selectOverExtension, y-selectOverExtension, width + 2*selectOverExtension, height + 2*selectOverExtension)
    }
    if(fill) {
      graphics.fill = color
  ...
  def getLineWidth = this.lineWidth
